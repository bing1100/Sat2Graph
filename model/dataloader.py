...
# angle is in degrees 
def rotate(sat_img, gt_seg, neighbors, angle=0, size=2048):
	...
	return sat_img, gt_seg, new_neighbors, mask
...
class Sat2GraphDataLoader():
	def __init__(self, folder, indrange = [0,10],imgsize = 256, preload_tiles = 64, max_degree = 6, loadseg = False, random_mask=True, testing=False, dataset_image_size = 924, transpose=False):
		self.folder = folder
		...
		self.input_sat = np.zeros((preload_tiles,image_size,image_size,3))
		
		self.gt_seg = np.zeros((preload_tiles,image_size,image_size,1))
		self.target_prob = np.zeros((preload_tiles,image_size,image_size,2*(max_degree + 1)))
		self.target_vector = np.zeros((preload_tiles,image_size,image_size,2*(max_degree)))
	...
	def loadtile(self, ind):
		...
		# if self.dataset_image_size == 256:
		# 	tiles_prob[0,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_prob256.png" % ind, dtype=int).reshape((1,256,256,2*(self.max_degree + 1)))
   		# 	tiles_vector[0,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_vect256.png" % ind, dtype=int).reshape((1,256,256,2*self.max_degree))
      	# 	return sat_img, tiles_prob, tiles_vector
    
		# if self.dataset_image_size == 352:
		# 	tiles_prob[0,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_prob352.png" % ind, dtype=int).reshape((1,352,352,2*(self.max_degree + 1)))
        #  	tiles_vector[0,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_vect352.png" % ind, dtype=int).reshape((1,352,352,2*self.max_degree))
        #   	return sat_img, tiles_prob, tiles_vector
	...
	def preload(self, num = 1024, seg_only = False):
		...
		for i in range(self.preload_tiles):
			...
			try:
				sat_img = scipy.ndimage.imread(self.folder + "/region_%d_sat.png" % ind)[:,:,:3]
			except:
				sat_img = scipy.ndimage.imread(self.folder + "/region_%d_sat.jpg" % ind)[:,:,:3]
			...
			if self.testing == False and random.randint(0,5) < 4:
				...
				sat_img, gt_seg, neighbors, rotmask = rotate(sat_img, gt_seg, neighbors, angle= angle, size = self.dataset_image_size)
			...
			# if self.dataset_image_size == 256:
			# 	tiles_prob[i,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_prob256.png" % ind, dtype=int).reshape((1,256,256,2*(self.max_degree + 1)))
   			# 	tiles_vector[i,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_vect256.png" % ind, dtype=int).reshape((1,256,256,2*self.max_degree))
			# elif self.dataset_image_size == 352:
			# 	tiles_prob[i,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_prob352.png" % ind, dtype=int).reshape((1,352,352,2*(self.max_degree + 1)))
			# 	tiles_vector[i,:,:,:] = np.loadtxt(self.folder + "/region_%d_refine_gt_graph_vect352.png" % ind, dtype=int).reshape((1,352,352,2*self.max_degree))
			# else:
			# 	self.tiles_input[i,:,:,:] = sat_img.astype(np.float)/ max_v - 0.5
			# 	self.tiles_gt_seg[i,:,:,0] = gt_seg.astype(np.float)/255.0 - 0.5
